...
import { listPaidPlans, resolveCheckoutPlan } from "@/lib/server/payment/config";
import { createPaymentOrder, markPaymentOrder } from "@/lib/server/payment/orders";
import { getStripeClient, isStripeAvailable } from "@/lib/server/payment/providers/stripe";
import Stripe from "stripe";

export function getSupportedCheckoutPlans() {
    return listPaidPlans();
}
...
export async function createCheckoutSession(userId: number, planId: PlanId) {
    ...
    const orderId = await createPaymentOrder({
        userId,
        planId,
        provider: serverEnv.devStripeMock || !isStripeAvailable() ? "mock" : "stripe",
        amount: plan.stripeUnitAmount ?? Math.round(plan.price * 100),
        currency: plan.currency ?? "usd",
        metadataJson: JSON.stringify({ plan: planId, credits: plan.credits }),
    });

    if (serverEnv.devStripeMock || !isStripeAvailable()) {
        await addCredits({
            userId,
            amount: plan.credits,
            source: "purchase",
            expiresAt: getExpiryDate(),
            details: { plan: planId, provider: "mock", order_id: orderId },
        });
        ...
        await markPaymentOrder({ orderId, status: "completed", providerReferenceId: `mock_${Date.now()}` });
        return { url: "/dashboard?payment=success&mock=1", mock: true, orderId };
    }
    ...
    try {
        ...
        await markPaymentOrder({ orderId, status: "pending", providerReferenceId: session.id });
        return { url: session.url ?? "/dashboard", orderId };
    } catch (error) {
        if (!serverEnv.allowMockFallback) {
            await markPaymentOrder({ orderId, status: "failed" });
            throw error;
        }
        await addCredits({
            userId,
            ...
            source: "purchase",
            expiresAt: getExpiryDate(),
            details: { plan: planId, provider: "stripe_fallback", order_id: orderId },
        });
        ...
        await markPaymentOrder({ orderId, status: "completed", providerReferenceId: `fallback_${Date.now()}` });
        return {
...
export async function handleStripeWebhook(payload: string, signature: string) {
    ...
    const event = await stripe.webhooks.constructEventAsync(payload, signature, serverEnv.stripeWebhookSecret);
            ...
            case "checkout.session.completed": {
                ...
                if (userId > 0 && credits > 0) {
                    await addCredits({
                        ...
                        source: "purchase",
                        expiresAt: getExpiryDate(),
                        details: { plan: planId, provider: "stripe", session_id: session.id, order_id: orderId || null },
                    });
                    ...
                    if (orderId > 0) {
                        await markPaymentOrder({ orderId, status: "completed", providerReferenceId: session.id });
                    }
                }
                break;
            }
...
export async function setPlanAdmin(userId: number, plan: string) {
    const supported = new Set(["free", ...getSupportedCheckoutPlans()]);
    if (!supported.has(plan as PlanId)) {
